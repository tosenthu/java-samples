...
public class AnagramCheck {
	...
	private static List<String> generateRandomString() {
		...
		for (int count = 0; count < Utility.nextInt(10, 15); count++) {
		...
		for (List<Character> characterSetList : preparedListOfCharacterSet) {
			int randomInt = Utility.nextInt(1, preparedListOfCharacterSet.size());
			returnValue.add(Utility.prepareStringFromCharacterArray(characterSetList));
			if ((randomInt % 5) == 0) {
				AnagramCheck.randomizeString(characterSetList, returnValue, Utility.nextInt(2, 10));
			} else if ((randomInt % 4) == 0) {
				AnagramCheck.randomizeString(characterSetList, returnValue, Utility.nextInt(2, 8));
			} else if ((randomInt % 3) == 0) {
				AnagramCheck.randomizeString(characterSetList, returnValue, Utility.nextInt(2, 6));
			}
		}
	...
	private static void randomizeString(List<Character> listOfCharacters, Set<String> returnValue, int randomLimit) {
		int randomInt = Utility.nextInt(1, randomLimit);
